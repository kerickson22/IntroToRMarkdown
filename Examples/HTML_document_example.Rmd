---
title: "Testing for differences between lambdas"
author: "KDE"
date: "June 25, 2019"
output: 
  html_document:
...
Load data, set the number of reshufflings to run and initialize a vector to store the values of $\lambda$. (Code not shown)
```{r loading data, eval=FALSE, include=FALSE}
...
##### Calculate $\lambda$ for each of the six sites using reshuffled data 
3 runs took 17331.39s 
10 runs took 58506.22s
Each run takes 5833.662s (~1 hour and 38 minutes)
```{r calculate lambda, eval = F, include=FALSE}
...
##### Calculate the pairwise differences between $\lambda$s 

Store them in a dataframe called 'differences': 
...
##### Calculate the observed pairwise differences in $\lambda$ between sites 

These differences are calculated using the values of $\lambda$ from running the analysis before the sites were shuffled: 

```{r calculate observed differences}
...
##### Plotting the distributions of differences

Can show these differences as either histograms or a cumulative distribution function. 
The function 'ecdf' is used to calculate the empirical cdf corresponding to each of the distributions of differences in $\lambda$. 
We then ask what percentage of the reshuffled differences are more extreme than our observed difference. 

The actual values will change as more samples of $\lambda$s calculated from the reshuffled data are added. 


```{r distributions of differences }
hist(differences[,1], main = "|BC - CC|", xlab="Difference")
abline(v=BC_CC_obs,col="red")

#Big Cypress - Cape Canaveral 
...
hist(differences[,1], main = "|BC - C|", xlab="Difference", xlim=c(0, 0.2))
abline(v=BC_C_obs, col="red")
text("topright", col="red", "0")
...
#Chekika - Fort Pierce 
hist(differences[,10], main="|C - FP|", xlab="Difference", xlim=c(0, 0.15))
...
#Chekika - Punta Gorda 
hist(differences[,11], main = "|C - PG|", xlab="Difference", xlim=c(0,0.15))
...
#Chekika - Wild Turkey
hist(differences[,12], main = "|C - WT|", xlab="Difference", xlim=c(0,0.16))
